...
public class KonvertiereVideo implements IKonvertiereVideo {
    ...
    public boolean konvertiereVideo(VideoTO aVideoTO) {
        ...
        try {
            logger.warning(
                    "Konvertierung des folgenden Videos steht bevor: " + aVideoTO.getDateipfad());
        ...
        for (Video vid : vids) {
            if (vid.getBeschreibung().equals(aVideoTO.getBeschreibung())
                ...
                if (!directory.exists()) {
                    ...
                    if (success) {
                        logger.warning(
                                "Anlegen des Ordners mit folgender ID war erfolgreich: "
                                        + vid.getId());
                    } else {
                        logger.warning(
                                "Fehler beim Anlegen des Ordners für die VideoId: " + vid.getId());
                    }
                } else {
